...
const SPRINT_MODES = ["all", "easy", "medium", "hard", "mixed"] as const;
...
export default async function LeaderboardPage({ searchParams }: PageProps) {
  ...
  const mode = SPRINT_MODES.includes(params.mode as (typeof SPRINT_MODES)[number])
    ? (params.mode as (typeof SPRINT_MODES)[number])
    : "all";
  ...
  if (board === "solved" || board === "topic") {
    ...
    let query = supabase
      ...
      .select("user_id, score, mode, questions_correct, questions_answered")
      .not("finished_at", "is", null);
    if (since) query = query.gte("started_at", since.toISOString());
    if (mode !== "all") query = query.eq("mode", mode);
    const { data } = await query;
    const best = new Map<string, { score: number; detail: string }>();
    for (const s of data ?? []) {
      const prev = best.get(s.user_id);
      if (!prev || s.score > prev.score) {
        best.set(s.user_id, {
          score: s.score,
          detail: `${s.questions_correct}/${s.questions_answered} · ${s.mode}`,
        });
      }
    }
    rows = [...best.entries()]
      .map(([userId, v]) => ({ userId, value: v.score, detail: v.detail }))
      .sort((a, b) => b.value - a.value);
    valueLabel = "Score";
  }
  ...
  const makeHref = (over: Partial<{ board: Board; window: Window; mode: string; topic: string }>) => {
    const p = new URLSearchParams();
    p.set("board", over.board ?? board);
    p.set("window", over.window ?? window);
    if ((over.board ?? board) === "sprint") p.set("mode", over.mode ?? mode);
    if ((over.board ?? board) === "topic") p.set("topic", over.topic ?? topic);
    ...
    <div className="space-y-6">
      ...
      {visible.length === 0 ? (
        ...
        <ul className="rounded-lg border border-border bg-card overflow-hidden">
          <li className="grid grid-cols-[3rem_minmax(0,1fr)_6rem] items-center gap-3 px-4 py-2 border-b border-border bg-muted/40 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            <span>#</span>
            <span>Player</span>
...
function LeaderRow({
  ...
  return (
    <li
      className={cn(
        "grid grid-cols-[3rem_minmax(0,1fr)_6rem] items-center gap-3 px-4 py-2.5 border-b border-border last:border-0",
        isYou && "bg-primary/10"
      )}
    >
